...
function secCheck($pth){
	echo "<br/>$pth";
	if(strtolower(urldecode(substr($pth,0,4))) == "file"){
		echo("security error");
		return false;
	}
	if(substr($pth,strlen($pth)-1,1) == "."){
		$pth = substr($pth,0,strlen($pth)-1);
	}
	echo "<br/>$pth";
	if(strtolower(urldecode(substr($pth,0,1))) == "/"){
		$pth = ".{$pth}";
	}
	echo "<br/>$pth";
	return $pth;
...
class FNOutput{
	...
	function folderList($folders){
		...
		for($i = 0; $i<count($folders);$i++){
			...
			if(!in_array($hPath,$this->ignoreFolders)){
				...
				$comment = "<span class=\"folder_comment\"> <a href=\"" . $_SERVER['PHP_SELF'] . "?dir=$hPath\"> ></a>";
	...
	function doFileProps($file,$path){
		...
		if(in_array(strtolower($ext),$this->imgTypes)){
			...
			<dd>" . $this->returnFileSize(filesize($path)) . "</dd>
			<dt>Options:</dt>
			<dd><a href=\"" . $_SERVER['PHP_SELF'] . "?src=$path\" title=\"view $file source code\">view source</a></dd>";
	...
	function doFileLink($file,$path){
		...
		$ext = substr(strrchr($file, '.'), 1);
		if(in_array(strtolower($ext),$this->imgTypes)){
			return "<a href=\"" . $_SERVER['PHP_SELF'] . "?view=$path\" title=\"view $file\">$file</a>";
		}else if(in_array(strtolower($ext),$this->embedTypes)){
			return "<a href=\"" . $_SERVER['PHP_SELF'] . "?view=$path\" title=\"view $file\">$file</a>";
		}else if(in_array(strtolower($ext),$this->phpTypes) || in_array(strtolower($ext),$this->htmlTypes)){
	...
	function doFileLinkInt($file,$path){
		...
		if(in_array(strtolower($ext),$this->imgTypes)){
			...
			return "<a href=\"" . $_SERVER['PHP_SELF'] . "?src=$path\" title=\"view $file source code\">$file</a>";
		}else if(in_array(strtolower($ext),$this->miscTypes)){
			return "<a href=\"$path\" title=\"download $file\">$file</a>";
		}
	}
	...
	function viewFile($file){
		...
		if(in_array($ext,$this->imgTypes)){
			...
			echo("\n<div id=\"imgWrapper\">\n<div id=\"imgPreview\">");
			if($preNext[0] != ""){
				echo("\n<a href=\"" . $_SERVER['PHP_SELF'] . "?view=".$preNext[0]."\">Prev</a>");
			}
			if($preNext[0] != "" && $preNext[1] != ""){
				echo(" | ");
			}
			if($preNext[1] != ""){
				echo("\n<a href=\"" . $_SERVER['PHP_SELF'] . "?view=".$preNext[1]."\">Next</a>");
			}
			echo "\n<br /><br />";
			if($preNext[1] != ""){
				echo("\n<a href=\"" . $_SERVER['PHP_SELF'] . "?view=".$preNext[1]."\"><img src=\"".$_GET['view']."\" width=\"".$imageDim[0]."\" height=\"".$imageDim[1]."\" alt=\"".$_GET['view']."\" /></a>\n<br /><br />");
			}else{
				echo("\n<img src=\"".$_GET['view']."\" /><br /><br />");
			}
			echo("\n<a href=\"" . $_SERVER['PHP_SELF'] . "?view=".$this->getFolder($file)."\" title=\"close image\">close</a>");
			echo("\n</div>\n</div>\n<br /><br />
...
class FNFileList{
	...
	function search($sstring){
		if(strlen($sstring)>0){
			...
			$f = $this->getFilesRecursive($this->searchArray,"./");
			$found = array();
