 ...
 * ML : This file replaces the entire Database.php in Kohana library -- it does not extend it.
 */
class Database_Core {
	...
	public    $set        = array(); // ML: allow it to be read by Model
	 ...
	 * Runs a query into the driver and returns the result.
         * ML added $bound_vars option for binding vars to straight SQL
	 *
	...
	public function query($sql = '', $bound_vars=array())
	{
		...
		// ML: following built-in bind features modified to start from index 2
		if (func_num_args() > 2) //if we have more than 2 argument ($sql)
		{
			$argv = func_get_args();
			$binds = (is_array($argv[2])) ? $argv[2] : array_slice($argv, 2);
		}
		...
		// Fetch the result
		$result = $this->driver->query(($this->last_query = $sql), $bound_vars); //added var binding feature - ML
	...
	public function query_row($sql = '', $bound_vars=array())
	{
		...
		// Fetch the result
		$result = $this->driver->query_row(($this->last_query = $sql), $bound_vars);
	...
	public function query_field($sql = '', $bound_vars=array())
	{
		...
		// Fetch the result
		$result = $this->driver->query_field(($this->last_query = $sql), $bound_vars);
	...
	public function procedure_call($sql, $bound_vars=array())
	{
		...
		// Fetch the result
		$result = $this->driver->procedure_call(($this->last_query = $sql), $bound_vars);
	 ...
	 * @return  Database_Core        This Database object.
	 */
	public function set($key, $value = '', $do_escape=TRUE) // ML: added do_escape option
	{
		if ( ! is_array($key))
		{
			$key = array($key => $value);
	 ...
	 */
	public function insert($table = '', $set = NULL, $bound_vars=array())
	{
		if ( ! is_null($set))
		{
			$this->set($set);
		...
		return $this->query($sql, $bound_vars);
	}
	 ...
	 * @return  Database_Result  Query result
	 */
	public function update($table = '', $set = NULL, $where = NULL, $bound_vars=array())
	{
		if ( is_array($set))
		{
			$this->set($set);
