...
//				and owner_id = :OWNER_ID', array(':OWNER_ID' => $owner_id)));
            return ($this->get_value("SELECT sum((principal_assessment - principal_waiver - principal_payment) +
                                                                (late_fee_assessment - late_fee_waiver - late_fee_payment) +
                                                                (interest_assessment - interest_waiver - interest_payment) + refund) VAL
                                FROM ustx.owner_transactions_view
                                WHERE fiscal_year > 1978
                                                and owner_id = :OWNER_ID", array(':OWNER_ID' => $owner_id)));
	}
	...
	public function waive_all($owner_id, $reason) {
		...
		$this->db->procedure_call("BEGIN ustx.invoice.waive_all(:p_owner_id, :p_reason, :p_user_id, :p_success); END;", array(':p_owner_id' => $owner_id, ':p_reason' => $reason, ':p_user_id' => $user_id, ':p_success' => $success));
	...
	/**
	 * get all the active and TOS tanks of owner
	 */
	public function get_owner_tanks($owner_id) {
		$owner_tanks = $this->db->query('
			SELECT * FROM USTX.tanks 
			WHERE owner_id = :OWNER_ID
			AND tank_status_code IN (1,2)'
