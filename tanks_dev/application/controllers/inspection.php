...
class Inspection_Controller extends Template_Controller {
	...
	public function download($inspection_id=NULL)
	{
		...
		$facility_name = htmlspecialchars(text::capitalize_first_letters($view->facility_row['FACILITY_NAME']));
		$facility_address = htmlspecialchars(text::capitalize_first_letters($view->facility_row['ADDRESS1']));
		...
		$owner_name = htmlspecialchars(text::capitalize_first_letters($view->owner_row['OWNER_NAME']));
		...
		// January 01, 2068 is null in detabase because UDAPI gives error when inserting null value
		$noirt_date = ($noirt_date == 'January 01, 2068') ? Null : $noirt_date;
		...
		$operator_name = htmlspecialchars(text::capitalize_first_letters($view->operator_row['OPERATOR_NAME']));
		...
		if($letter_name == 'NOD' || $letter_name == 'NOIRTB' || $letter_name == 'RTPIDB' || $letter_name == 'STANDARD_NOV' || $letter_name == 'ELEVATED_NOV' || $letter_name == 'NOMV') {
			$document->cloneRow('Violation_Description', count($penalties_b));
		
			foreach($penalties_b as $key => $penalty) {
				$row_no = $key + 1;
				$ustr = is_null($penalty['USTR']) ? '' : $penalty['USTR'] . '.';
				$violation_code_description = Model::instance('Penalty_codes')->get_lookup_desc($penalty['PENALTY_CODE'], FALSE);
				$position = strpos($violation_code_description, ':');
				$violation_description = substr($violation_code_description, $position + 2);
				$document->setValue('Violation_Code#' . $row_no, $ustr . $penalty['PENALTY_CODE'] . ' NMAC');
