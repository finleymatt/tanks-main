...
class PHPExcel_Writer_PDF_tcPDF extends PHPExcel_Writer_PDF_Core implements PHPExcel_Writer_IWriter
{
	...
	//  Create PDF
	// MINLEE: Use custom extended class if exists -- must be named 'MY_TCPDF'
	if (class_exists('MY_TCPDF')) {
		$pdf = new MY_TCPDF($orientation, 'pt', $paperSize);
	}
	else {
	...
	//$pdf->setFontSubsetting(TRUE);  // MINLEE: set to TRUE to include fonts
        $pdf->AddPage();

        //  Set the appropriate font
	//$pdf->SetFont($this->getFont());
        $pdf->writeHTML(
		//$this->generateHTMLHeader(FALSE) . MINLEE
            strtr($this->generateSheetData(), array('<style>' => '<!-- ', '</style>' => ' -->'))
            //$this->generateHTMLFooter() MINLEE
        );
