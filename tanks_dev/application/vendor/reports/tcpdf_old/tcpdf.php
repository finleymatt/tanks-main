...
$k_path_url = dirname(__FILE__); // MINLEE added
			...
			while ($elkey < $maxel) {
				...
				if (preg_match($tagpattern, $element)) {
					...
					if ($element{0} == '/') {
						...
						// change each() to foreach(), each() has been DEPRECATED as of PHP 7.2.0, and REMOVED as of PHP 8.0.0
			...
			switch($tag['value']) {
				...
				case 'img': {
					if (isset($tag['attribute']['src'])) {
						...
						/******** by Min Lee - commented out because the relative path for PDF doesnt work
