...
//if ($nov_type == 'SOC')
//	$filter_sql = "PC.IS_SOC = 'T'";
//else
	$filter_sql = "PC.DP_CATEGORY IS NOT NULL";

$tank_types[] = 'B';  // add Both
$tank_type_list = implode( ',', array_map(function($val) { return("'{$val}'");}, $tank_types) );

$report_sql = "
	SELECT F.id facility_id, F.facility_name, O.id owner_id, O.owner_name, I.date_inspected, IC.description inspection_type, P.penalty_code, PC.description penalty_description, PC.tcr_category, PC.dp_category
...
$caf_report = new Report($output_format, 'NOV Report - Delivery Prohibition', "Dates: {$start_date} to {$end_date}\nTank Types: " . implode(', ', $tank_type_names));
...
// labels -----------------------------------------------
$caf_report->setLabelRow( array('NOV Code', 'NOV Description', 'DP Category', 'TCR Category', 'Date Inspected', 'Inspection Type', 'Owner ID', 'Owner Name', 'Facility ID', 'Facility Name', 'AST', 'UST'), array('style' => array('alignment' => array('wrap' => TRUE))) );
...
if (count($rs_arr)) {
	...
	foreach ($rs_arr as $row) {
		...
		$caf_report->setRow(array(
			...
			array('value' => $row['DP_CATEGORY'], 'style' => array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER))),
			array('value' => $row['TCR_CATEGORY'], 'style' => array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER))),
	...
	// summary -----------------------------------------------
	$group_row_end = $caf_report->row_num - 1;
	$caf_report->setRow(array(
		array('value' => 'NOV Count:'),
		array( 'value' => "=COUNT(A{$group_row_start}:A{$group_row_end})" ),
		array( 'colspan' => 6 ),
...
$caf_report->setColumnSize(array(12, 35, 12, 15, 12, 20, 15, 35, 15, 35, 8, 8));
$flag = $caf_report->output('nov_report');
