 ...
 * imported from Oracle Report, "Facilities not Inspected in Last 18 Months"
 *
 * @package Onestop
 * @subpackage views
 * @uses Report.php
...
$report_sql = "
	...
	WHERE
		(select count(*) from ustx.tanks T where (T.facility_id = F.id) and T.TANK_TYPE = 'U' and (t.tank_status_code in (1, 2))) > 0
		AND Upper(CT.county) = Upper({$county_sql})
		AND trunc(sysdate - I.last_inspected_date) > 548
...
$report = new Report($output_format, 'Eighteen Months No Compliance Report', "Tanks with no inspections since {$before_date}\nCounty: ". ($county ? $county : 'All Counties') ."\nReport Generated on: " . date('m/d/Y'));
...
if (count($rs_arr)) {
	...
	foreach ($rs_arr as $row) {
		...
		if ($row['DAYS_SINCE'] >= 730)
			$overdue_style =  array('fill' => array(
				'type' => PHPExcel_Style_Fill::FILL_SOLID,
				'color' => array('argb' => 'FF0000')));
		elseif ($row['DAYS_SINCE'] >= 548 && $row['DAYS_SINCE'] < 730)
			$overdue_style =  array('fill' => array(
				'type' => PHPExcel_Style_Fill::FILL_SOLID,
...
$report->setColumnSize(array(8, 35, 35, 20, 6, 10, 18, 8, 8, 8, 16, 16, 13));
$flag = $report->output('18_month_no_comp');
?>
