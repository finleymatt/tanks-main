...
if (count($rs_arr)) {
	...
	foreach ($rs_arr as $row) {
		...
		// replace '&AMP;' with '&' until data in database are changed
		$facility_name = str_replace("&AMP;", "&", $row['FACILITY_NAME']);
		$facility_address = str_replace("&AMP;", "&", $row['ADDRESS1']);
		$owner_name = str_replace("&AMP;", "&", $row['OWNER_NAME']);

		$caf_report->setRow(array(
			array('value' => $row['FID']),
			array('value' => $facility_name),
			array('value' => $facility_address),
			...
			array('value' => $owner_name, 'style' => array('alignment' => array('wrap' => TRUE)))
		));
	}
